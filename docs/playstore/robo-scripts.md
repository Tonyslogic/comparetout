...
| **Tab tour — at scroll-top, bars visible** | `text="Comparisons"` → open `text="Sources"` (top accordion, no scroll) · `text="Directors"` → `contentDescription="Menu"` → `text="Supplier Plans"` → `PRESSED_BACK` · `text="Scenarios"` → `contentDescription="Options"` → `text="Edit"` → wizard section tour → `PRESSED_BACK` | Every bottom-nav tab is visited while nothing has scrolled, so each tap has an on-screen target. `Sources` is the only accordion opened here (top of Compare, a tap not a scroll). The wizard opens on the **populated** sample scenario (`Options → Edit`), so its sections are unlocked. See the wizard note below. |
| **↳ Wizard section tour (`ELEMENT_SCROLL_INTO_VIEW` → open→close)** | For each of `Start · Usage Data · Inverters · PV System · Battery · Hot Water · EV · Heat Pump`: `ELEMENT_SCROLL_INTO_VIEW` (container `resourceId="wizard_section_scroll"`, child = the `Wizard section: …` cd) → `VIEW_CLICKED` (open) → `WAIT` → `VIEW_CLICKED` same selector (**close**) → `WAIT`. Every one of these is marked `"optional": true`. | Robo does **not** auto-scroll a scripted `VIEW_CLICKED` to an off-screen target (only `RecyclerView`/`AdapterView` children get auto-positioning; a Compose `Column.verticalScroll` does not). It *will* scroll deterministically via `ELEMENT_SCROLL_INTO_VIEW`, but that needs the scroll container's `resourceId` — which Compose only exposes through `testTag` when `testTagsAsResourceId = true`. So the wizard's scroll `Column` now carries `.semantics { testTagsAsResourceId = true }.testTag("wizard_section_scroll")` (`UI2WizardActivity.kt`), and each section is scrolled into view before its tap. `Heat Pump` (bottom-most, `:714`, after EV) is now reachable this way. Everything is `optional` so a profile-gated-absent section (`uiVis.*` false) is skipped, never fatal — this replaced an earlier blind-`VIEW_SWIPED` approach that couldn't reliably bring `Heat Pump` into view on a phone (landscape shows only one accordion at a time). Screenshots are the ambient captures at each open. |
| **Terminal deep-dive — Dashboard, scroll-heavy, no tab tap after** | `text="Dashboard"` (tab, from the Scenarios list at scroll-top) → `text="Tariff Plan"` → `text="Explore data"` → `contentDescription="View graphs"` → `PRESSED_BACK` → `ELEMENT_SCROLL_INTO_VIEW`(`dashboard_scroll`, `KPIs`) → `text="KPIs"` → `ELEMENT_SCROLL_INTO_VIEW`(`dashboard_scroll`, `Visual overview`) → `text="Visual overview"` | The four Dashboard accordions in on-screen order. `View graphs` only renders while `Explore data` is expanded. `KPIs` and `Visual overview` sit below the fold, so each is scrolled in via `ELEMENT_SCROLL_INTO_VIEW` on the Dashboard's `.testTag("dashboard_scroll")` scroll `Column` (`UI2DashboardFragment.kt`) before its tap (both `optional`). Because no bottom-nav tab is tapped after this, the hidden-bar failure mode is designed out. |
...
> **Action numbers:** the table above is a *phase-level* narrative. The JSON counts each `WAIT`, `WAIT_FOR_ELEMENT`, `ELEMENT_SCROLL_INTO_VIEW`, and `VIEW_CLICKED` as its own action (90 total; the 2 leading conditional dialog handlers are a separate pool FTL excludes from its linear "of N" count), so a "Robo script failed on action N of M" message must be cross-referenced against [`walkthrough.json`](../../playstore/robo-scripts/walkthrough.json) itself (the authoritative action list), not this table. Note that `"optional": true` actions (the whole wizard tour + the Dashboard KPIs/Visual-overview taps) are **skipped, not fatal**, when their target isn't found.
...
| `resourceId="wizard_section_scroll"` / `resourceId="dashboard_scroll"` | Removing/renaming the `.testTag(...)` on the wizard or dashboard scroll `Column`, or dropping the `.semantics { testTagsAsResourceId = true }` that exposes it — breaks every `ELEMENT_SCROLL_INTO_VIEW` that reaches off-screen accordions | [`UI2WizardActivity.kt`](../../app/src/main/java/com/tfcode/comparetout/ui2/UI2WizardActivity.kt) `WizardScreen` scroll Column, [`UI2DashboardFragment.kt`](../../app/src/main/java/com/tfcode/comparetout/ui2/UI2DashboardFragment.kt) `DashboardScreen` scroll Column |
