...
public abstract class AlphaEssDAO {
    ...
    @Query("SELECT sum(pv) as PV, sum(load) AS LOAD, sum(feed) AS FEED, sum(buy) AS BUY, " +
            ...
            "0 AS EVSCHEDULE, 0 AS EVDIVERT, 0 AS HWSCHEDULE, 0 AS HWDIVERT, " +
    ...
    /**
     * Synchronous lookup of the transform meta row for a system. Returns {@code null}
     * if the row has never been written — treat that as v1 (pre-enrichment).
     */
    @Query("SELECT * FROM alphaESSTransformMeta WHERE sysSn = :sysSN")
    public abstract AlphaESSTransformMeta getTransformMeta(String sysSN);

    /** Insert-or-replace the per-SN transform stamp. */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    public abstract void upsertTransformMeta(AlphaESSTransformMeta meta);

    /**
     * Live snapshot of all transform meta rows. The UI joins this against the set of
     * known sysSn cards to decide whether to surface a Migrate button (missing row
     * or {@code transformVersion < TRANSFORM_VERSION_CURRENT} → stale).
     */
    @Query("SELECT * FROM alphaESSTransformMeta")
    public abstract LiveData<List<AlphaESSTransformMeta>> observeAllTransformMeta();
}
